...
class Net(torch.nn.Module):
    def __init__(self,in_feats, hidden_feats, out_feats, dropout, alpha, num_layers):
        super(Net, self).__init__()
        self.dropout = dropout
        self.conv1 = GATv2Conv_multi(in_feats, hidden_feats, dropout, alpha)
        
        self.hidden_layers = nn.ModuleList()
        for _ in range(num_layers - 2):
            self.hidden_layers.append(GATv2Conv_multi(hidden_feats, hidden_feats, dropout, alpha))
        
        self.conv2 = GATv2Conv_multi(hidden_feats, out_feats, dropout, alpha)
