...
struct mmaDenseTile_fp16_ori{
    ...
    struct mmaDenseTile_tf32_v2{
        ...
        float *sparse_fragment):
            ...
            matrix_base_(matrix + offset),
            //8的意思是vector的长度
            values_(reinterpret_cast<const int *>(values + row_offset_vec*2) + (lane_id & 31)),
        ...
        __device__ __forceinline__ void Fetch(int colEdge, int dimN_index){
            ...
            int sparse_fragment_l =__ldg(values_);
            if (sparse_fragment_l >= 0)
                sparse_fragment_[0]=__ldg(csr_values_ + sparse_fragment_l);
            else sparse_fragment_[0]=0.0;
            const long row_offsets_ = __ldg(column_idxs_);
            values_ += 32;
            column_idxs_ += 4;
            // (warp_id<<4) 每个warp有16列
    ...
    struct mmaDenseTile_fp16_map{
        ...
        float *sparse_fragment):
            ...
            matrix_base_(reinterpret_cast<const half2 *>(matrix + offset)),
            // row_offsets_base_(row_offsets),
            values_(reinterpret_cast<const int2 *>(values + row_offset_vec*2) + (lane_id & 31)),
            csr_values_(csr_values),
            column_idxs_(column_idxs + row_offset_vec + (((lane_id & 31)%4)*2)),
            dense_tile_(reinterpret_cast< half *>(dense_tile)),
            sparse_fragment_(reinterpret_cast< half *>(sparse_fragment))
            {}
    
        __device__ __forceinline__ void Fetch(int colEdge, int dimN_index){

            int2 sparse_fragment_l =__ldg(values_);
            if (sparse_fragment_l.x >= 0)
                sparse_fragment_[0]=__ldg(csr_values_ + sparse_fragment_l.x);
            else sparse_fragment_[0]=0.0;
            if (sparse_fragment_l.y >= 0)
                sparse_fragment_[1]=__ldg(csr_values_ + sparse_fragment_l.y);
            else sparse_fragment_[1]=0.0;
    ...
    // };
        ...
        float *sparse_fragment):
            ...
            warp_id(lane_id>>5),
            //每行16个线程，每个线程搬运1个double,
            matrix_base_(reinterpret_cast<const half *>(matrix)),
            // row_offsets_base_(row_offsets),
            values_(reinterpret_cast<const int2 *>(values + row_offset_vec*2) + (lane_id & 31)),
            csr_values_(csr_values),
            column_idxs_(column_idxs + row_offset_vec + (((lane_id & 31)%4)*2)),
        ...
        __device__ __forceinline__ void Fetch(int colEdge, int dimN_index){
            ...
            for(int i=0;i<2;i++)
            {
                const int global_offset = dimN_index + (warp_id<<4) + (warpin_id/4) + (i*8);
