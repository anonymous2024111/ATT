...
print(max)
result = ATT_SDDMM_v3.att_tf32_h_nnz(
row_pointers, 
column_index,
degrees, 
window,
...
if torch.dot(rhs[0][0],weight[0][0]) + torch.dot(rhs[0][1],weight[0][0])!= result[0][0] :
    print("No")
    exit(0)

if head>1:
    if torch.dot(rhs[1][0],weight[1][0]) + torch.dot(rhs[1][2],weight[1][0])!= result[1][1] :
        print("No")
        exit(0)
print("PASS")