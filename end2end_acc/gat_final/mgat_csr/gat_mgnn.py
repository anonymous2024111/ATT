...
class Net(torch.nn.Module):
    def __init__(self,in_feats, hidden_feats, out_feats, dropout, alpha, heads, num_layers):
        ...
        self.conv1 = GATConv_multi(in_feats, hidden_feats, dropout, alpha, heads)
        
        self.hidden_layers = nn.ModuleList()
        for _ in range(num_layers - 2):
            self.hidden_layers.append(GATConv_multi(hidden_feats*heads, hidden_feats, dropout, alpha, heads))
        
        self.conv2 = GATConv_multi(hidden_feats*heads, out_feats, dropout, alpha, 1)
