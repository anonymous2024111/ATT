...
rhs = torch.randint(low=1, high=3, size=(rows, head, dimN)).half()
weight = torch.ones((head, 1,dimN), dtype=torch.float16)
...
print(max)
result = ATT_SDDMM_v4.att_fp16_h_nnz(
row_pointers, 
column_index,
degrees, 
window,
...
if torch.dot(rhs[0][0],weight[0][0]) + torch.dot(rhs[1][0],weight[0][0])!= result[0][0] :
    print("No")
    exit(0)

if head>1:
    if torch.dot(rhs[0][1],weight[1][0]) + torch.dot(rhs[2][1],weight[1][0])!= result[1][1] :
        print("No")
        exit(0)
print("PASS")