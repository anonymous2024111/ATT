...
class GAT(nn.Module):
    def __init__(self, in_size, hid_size, out_size, heads, num_layers):
        super().__init__()
        self.dropout = 0.5
        self.conv1 = GATv2Conv(in_size, hid_size, heads)   

        self.hidden_layers = nn.ModuleList()
        for _ in range(num_layers - 2):
            self.hidden_layers.append(GATv2Conv(hid_size*heads, hid_size, heads))
                
        self.conv2 = GATv2Conv(hid_size*heads, out_size, 1)
