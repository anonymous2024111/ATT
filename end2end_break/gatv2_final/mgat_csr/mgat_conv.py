...
class MGATFunction_multi(torch.autograd.Function):
    ...
    def forward(ctx, X_prime0, X_prime1, a0, inputInfo):
        ...
        att = ATT_SDDMM_v1.attv2_fp16_h_nnz(
        ...
        with open("/home/shijinliang/module/tpds/ATT/end2end_break/gatv2_final/result/mgat16.csv", "a") as f:
    ...
    def backward(ctx, att_grad):
        ...
        求a0梯度
        '''
        a0_right = ATT_SpMM_v1.spmm_fp16_h_m_rowsum(
            inputInfo.row_pointers,
        ...
        a0_left = ATT_SpMM_v1.spmm_fp16_h_m_rowsum(
            inputInfo.row_pointers,
            inputInfo.degrees, 
            att_grad,
            inputInfo.window,
...
class MGATSpmm_multi(torch.autograd.Function):
    ...
    def forward(ctx, att, X_prime, inputInfo):
        ...
        torch.cuda.synchronize()
        start_time = time.time()
        X_prime= ATT_SpMM_v1.spmm_fp16_h_m(
            inputInfo.row_pointers,
            inputInfo.column_index, 
            inputInfo.degrees, 
            att,
    ...
    def backward(ctx, X_prime_grad):
        ...
        d_X_prime= ATT_SpMM_v1.spmm_fp16_h_m(
            inputInfo.row_pointers,
            inputInfo.column_index, 
            inputInfo.degrees_trans, 
            att,
        ...
        d_att = ATT_SDDMM_v1.sddmm_fp16_h_nnz(
            inputInfo.row_pointers, 
            inputInfo.column_index,
            inputInfo.degrees, 
            inputInfo.window,
...
class MGATSpmm1_multi(torch.autograd.Function):
    ...
    def forward(ctx, att, inputInfo):
        ...
        X_prime= ATT_SpMM_v1.spmm_fp16_h_m_rowsum(
            inputInfo.row_pointers,
            inputInfo.degrees, 
            att,
            inputInfo.window,
    ...
    def backward(ctx, X_prime_grad):
        ...
        d_att = ATT_SDDMM_v1.sddmm_fp16_ones_h_nnz(
            inputInfo.row_pointers, 
            inputInfo.column_index,
            inputInfo.degrees, 
            inputInfo.window,
...
class GATv2Conv_multi(torch.nn.Module):
    def __init__(self, input_dim, output_dim, dropout, alpha):
        ...
        # self.fc0 = nn.Linear(input_dim, output_dim, bias=False)
    ...
    def forward(self, X, inputInfo):
        ...
        X_prime0 = torch.mm(X, self.w0.half())
        X_prime1 = torch.mm(X, self.w1.half())
