...
class MGAT_dataset_csr(torch.nn.Module):
    ...
    def init_edges(self):
        ...
        self.dd1=torch.tensor(dd, dtype=torch.float32) 
        dd= torch.rsqrt(self.dd1).to(torch.float16)  
        
        self.dd1=self.dd1[:self.num_nodes_ori].reshape(-1, 1).to(torch.float16)  
                
        # 计算每行的非零元素个数
        row_nonzero_counts = adj.indptr[1:] - adj.indptr[:-1]

        # 将结果转换为形状为 (m, 1) 的 PyTorch 张量
        row_nonzero_counts_tensor = torch.tensor(row_nonzero_counts, dtype=torch.int).view(-1, 1)
