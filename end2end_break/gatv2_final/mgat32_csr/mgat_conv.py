...
class MGATFunction_multi(torch.autograd.Function):
    ...
    def forward(ctx, X_prime0, X_prime1, a0, inputInfo):
        ...
        att = ATT_SDDMM_v1.attv2_tf32_h_nnz(
        ...
        with open("/home/shijinliang/module/tpds/ATT/end2end_break/gatv2_final/result/mgat32.csv", "a") as f:
            f.write(str(round(execution_time,4)*1000) + ',')
        return att
    ...
    def backward(ctx, att_grad):
        ...
        求a0梯度
        '''
        a0_right = ATT_SpMM_v1.spmm_tf32_h_m_rowsum(
            inputInfo.row_pointers,
        ...
        a0_grad_right = torch.mm(a0_right.transpose(0, 1), X_prime1)

        a0_left = ATT_SpMM_v1.spmm_tf32_h_m_rowsum(
            inputInfo.row_pointers,
            inputInfo.degrees, 
            att_grad,
            inputInfo.window,
            inputInfo.automic,
            inputInfo.num_nodes_ori)[0]
        a0_grad_left = torch.mm(a0_left.transpose(0, 1), X_prime0)
        
        # 求X_prime_grad
        X_prime_grad_right = torch.mm(a0_right, a0) 
        X_prime_grad_left = torch.mm(a0_left, a0)
...
class MGATSpmm_multi(torch.autograd.Function):
    ...
    def forward(ctx, att, X_prime, inputInfo):
        ...
        torch.cuda.synchronize()
        start_time = time.time()
        X_prime= ATT_SpMM_v1.spmm_tf32_h_m(
            inputInfo.row_pointers,
            inputInfo.column_index, 
            inputInfo.degrees, 
            att,
    ...
    def backward(ctx, X_prime_grad):
        ...
        d_att = ATT_SDDMM_v1.sddmm_tf32_h_nnz(
            inputInfo.row_pointers, 
            inputInfo.column_index,
            inputInfo.degrees, 
            inputInfo.window,
...
class MGATSpmm1_multi(torch.autograd.Function):
    ...
    def forward(ctx, att, inputInfo):
        ...
        X_prime= ATT_SpMM_v1.spmm_tf32_h_m_rowsum(
            inputInfo.row_pointers,
            inputInfo.degrees, 
            att,
            inputInfo.window,
    ...
    def backward(ctx, X_prime_grad):
        ...
        d_att = ATT_SDDMM_v1.sddmm_tf32_ones_h_nnz(
            inputInfo.row_pointers, 
            inputInfo.column_index,
            inputInfo.degrees, 
            inputInfo.window,
...
class GATv2Conv_multi(torch.nn.Module):
    def __init__(self, input_dim, output_dim, dropout, alpha, head):
        ...
        self.w0 = torch.nn.Parameter(torch.zeros(size=(input_dim, output_dim)))
        nn.init.xavier_normal_(self.w0.data, gain=gain1)

        self.w1 = torch.nn.Parameter(torch.zeros(size=(input_dim, output_dim)))
        nn.init.xavier_normal_(self.w1.data, gain=gain1)  
    ...
    def forward(self, X, inputInfo):
        #特征降维
        X_prime0 = torch.mm(X, self.w0)
        X_prime1 = torch.mm(X, self.w1)
        X_prime0 = self.leakyrelu(X_prime0)
        X_prime1 = self.leakyrelu(X_prime1)
        
        #求att和用于反向传播的att_trans
        att = MGATFunction_multi.apply(X_prime0, X_prime1, self.a0, inputInfo)
