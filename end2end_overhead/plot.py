...
patterns = ['#', '\\', '/', '\\', '|-']
# GAT16 堆叠两层（左边）
plt.barh(y - bar_height / 2, gat16_pre, height=bar_height, label='GAT16-Forward', color=colors[2])
plt.barh(y - bar_height / 2, gat16_train, height=bar_height, left=gat16_pre, label='GAT16-Backward', color=colors1[1])

# GAT32 堆叠两层（右边）
plt.barh(y + bar_height / 2, gat32_pre, height=bar_height, label='GAT32-Forward', color=colors[2])
plt.barh(y + bar_height / 2, gat32_train, height=bar_height, left=gat32_pre, label='GAT32-Backward', color=colors1[2])
