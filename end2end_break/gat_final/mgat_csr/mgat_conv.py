...
class MGATFunction_multi(torch.autograd.Function):
    ...
    def forward(ctx, X_prime, a0, a1, inputInfo):
        ...
        for i in range(100):
            att = ATT_SDDMM_v1.att_fp16_h_nnz(
    ...
    def backward(ctx, att_grad):
        ...
        求a0,a1梯度
        '''
        a0_tmp= ATT_SpMM_v1.spmm_fp16_h_m_rowsum(
            inputInfo.row_pointers,
        ...
        #att_grad求a1梯度
        a1_tmp= ATT_SpMM_v1.spmm_fp16_h_m_rowsum(
            inputInfo.row_pointers,
            inputInfo.degrees, 
            att_grad,
            inputInfo.window,
...
class GATConv_multi(torch.nn.Module):
    ...
    def forward(self, X, inputInfo):
        #特征降维
        X_prime = torch.mm(X, self.w0.half())
