...
// tf32 - ns
    struct mmaDenseTile_tf32_ns{
        ...
        double *dense_tile_array):
            ...
            dense_tile_(reinterpret_cast< half *>(dense_tile)),
            sparse_fragment_(sparse_fragment),
            shared_dense(dense_tile_array + 16)
            {}
        ...
        __device__ __forceinline__ void Fetch(int colEdge, int dimN_index){
            ...
            sparse_fragment_[0]=__ldg(values_);
            values_ += 32;
            // at::Half dense_tile_half[4]={0.0,0.0,0.0,0.0};
            int col_temp = __ldg(column_idxs_ );

            // 搬运稠密数据
            shared_dense[warpin_id] = __ldg(matrix_base_ + rhs_cols_*col_temp/4 + warp_id*4 + (warpin_id%4));
            __syncwarp();
            dense_tile_ = reinterpret_cast< half *>(shared_dense);
            int global_offset = (warp_id%4)*2*16 + (warpin_id/4);
            dense_tile_[0]= *(dense_tile_ + global_offset);
            dense_tile_[2]= *(dense_tile_ + global_offset + 8);
            dense_tile_[1]= *(dense_tile_ + global_offset + 24);
            dense_tile_[3]= *(dense_tile_ + global_offset + 36);
            column_idxs_ += 8;
        }
